#!/u/achoi2/miniconda3/envs/vina/bin/python
...
if args.skip_repair:
    ...
    #Check if the repair step removed any residues
    #get the original PDB list and residues
    pdbs_list = [ os.path.join(main_dir, pdb) for pdb in pdbs_list ]
    original_residues_dictionary = {}
    
    for pdb in pdbs_list:
        try:
            key = os.path.splitext(os.path.basename(pdb))[0]
            original_residues_dictionary[key] = get_foldx_sequence(pdb, multimers=args.multimers)
        except IOError:
            exit(1)
    ...
    #get the repaired PDB list and residues
    residues_dictionary = {}
    for pdb in repaired_pdbs_list:
        try:
            key = os.path.basename(pdb)[:-11]
            value = get_foldx_sequence(pdb, multimers=args.multimers)
            residues_dictionary[key] = value
        except IOError:
            exit(1)
        except ValueError as e:
            log.error(f"{e}. Exiting...")
    ...
    #collect the missing residue errors for all models in run
    error_messages = []
    for pdb, residues in original_residues_dictionary.items():
        repaired_residues = residues_dictionary.get(pdb)
        if len(residues) != len(repaired_residues):
            difference = list(set(residues) - set(repaired_residues))
            error_messages.append(f"The following residues were removed during the repair {difference} on {pdb}. ")
    
    #log errors
    if error_messages:
        for error_message in error_messages:
            log.error(error_message)
        log.error("some residues were removed during repair; please check your PDB files for integrity and compatibility with FoldX. Exiting...")
        exit(1)
...
if args.poslist is not None:
    ...
    try:
        poslist = parse_poslist_file(args.poslist, unique_residues)
...
# this is currently only applicable when unrecognized molecules are found.
# remove failed runs from unique_residues
# e.g., if mutation at position 45A failed, remove 45A from unique_residues
for i in [x for x in mutate_outcome if x[1] == False]:
    failed_name = i[0]
    log.warning("Removing failed mutation run %s from residue list" % failed_name)
    print("failed name:", failed_name)
    print("unique residues before removal:", unique_residues )
    unique_residues.remove((failed_name,))

print("resulting unique residues:", unique_residues )
